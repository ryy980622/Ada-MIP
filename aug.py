...
# from SE_augmentation import SE_augment
from arguments import arg_parse
import json
# from SE_augmentation import batch_com, init, cnt_node, cnt_com
...
class TUDataset_aug(InMemoryDataset):
    ...
    def get(self, idx):
        # from gsimclr import data2aug, get_value, set_value
        # from SE_augmentation import batch_com
        # global data2aug, batch_com
        data = self.data.__class__()
        ...
        if data.edge_attr is not None:
            el = torch.zeros(node_num, data.edge_attr.shape[1])
            data.edge_attr = torch.cat((data.edge_attr, el), dim=0)
        data.edge_index = torch.cat((data.edge_index, sl), dim=1)
        data_com = copy.deepcopy(data)
        ...
        if self.aug == 'dnodes':
        ...
        elif self.aug == 'SE_augment':
            if idx in data2aug.keys():
                edge_index = get_value(data2aug, idx)
                edge_index = torch.tensor(edge_index)
                edge_index_com = get_value(data2com, idx)
                edge_index_com = torch.tensor(edge_index_com)
                #print(edge_index)
                data_aug = copy.deepcopy(data)
                data_aug.edge_index = edge_index
                data_com = copy.deepcopy(data)
                data_com.edge_index = edge_index_com

            else:
                edge_index, data_aug, edge_index_com, data_com = SE_augment(data)
                edge_index = [x.numpy().tolist() for x in edge_index]
                set_value(data2aug, idx, edge_index)
                set_value(data2com, idx, edge_index_com)
                #print(data2aug)
                #print(edge_index)

                #gsimclr.data2aug[data] = data_aug
            ...
            data_aug1 = drop_nodes(deepcopy(data))
            data_aug2 = permute_edges(deepcopy(data))
            data_aug3 = subgraph(deepcopy(data))
            data_aug4 = mask_nodes(deepcopy(data))
        else:
        ...
        if self.aug == 'all':
            return data, data_aug1, data_aug2, data_aug3, data_aug4
        elif self.aug == 'SE_augment':
            return data, data_aug, data_com
        else:
...
def drop_nodes(data):
    ...
    edge_nondrop = [i for i, edge in enumerate(edge_index.T) if edge_index[0][i] not in idx_drop and edge_index[1][i] not in idx_drop]

    edge_index = edge_index[:, edge_nondrop]
    '''
    ...
    if data.edge_attr is not None:
        ...
        edge_attr = edge_attr[edge_nondrop, :]
...
def permute_edges(data):
    ...
    edge_reserved = np.random.choice(edge_num, edge_num-permute_num, replace=False)
    edge_index = edge_index[edge_reserved]
    # edge_index = [edge_index[n] for n in range(edge_num) if not n in np.random.choice(edge_num, permute_num, replace=False)] + idx_add
    data.edge_index = torch.tensor(edge_index).transpose_(0, 1)
    if data.edge_attr is not None:
        data.edge_attr = data.edge_attr[edge_reserved]
    return data
...
def subgraph(data):
    ...
    edge_nondrop = [i for i, edge in enumerate(edge_index.T) if
                    edge_index[0][i] not in idx_drop and edge_index[1][i] not in idx_drop]
    # data.x = data.x[idx_nondrop]
    # edge_index = data.edge_index.numpy()
    edge_index = data.edge_index.transpose(0, 1).numpy()
    edge_index = edge_index[edge_nondrop]
    data.edge_index = torch.tensor(edge_index).transpose_(0, 1)
    if data.edge_attr is not None:
        data.edge_attr = data.edge_attr[edge_nondrop]
...
def init():
    global data2aug, data2com
    data2aug = {}
    data2com = {}
def set_value(dic, arg,value):
    dic[arg] = value
def get_value(dic, arg):
    return dic[arg]
...
init()
args = arg_parse()
with open('data/'+args.DS+'.json', 'r', encoding='UTF8') as f:
    data2aug = json.load(f)
'''
